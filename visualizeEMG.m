...
use_default_limits = input('Would you like to use the default y-axis limits ([-1 1; -1 1; -1 1; -.5 .5] volts)? yes(1) or no(0): ');
%ylims = [-1 1; -1 1; -1 1; -.5 .5];
...
for n=1:subplot_number
    ...
    %title_text = sprintf('Sweep #: %d', a); %%%%MF
    if n==1
...
EMGdata.trials.ch1_EMG_RT(a,1) = EMGdata.trials.ch1_EMGburst_onset(a,1) - EMGdata.trials.go_stim_onset(a,1); %%%%%%MF
...
EMGdata.trials.ch2_EMG_RT(a,1) = EMGdata.trials.ch2_EMGburst_onset(a,1) - EMGdata.trials.go_stim_onset(a,1); %%%%%MF
...
EMGdata.trials.ch3_EMG_RT(a,1) = EMGdata.trials.ch3_EMGburst_onset(a,1) - EMGdata.trials.go_stim_onset(a,1);
EMGdata.trials.ch2_EMGburst_area(a,1) = sum(abs(EMGrange)) / (round(x_new(2) * EMGdata.parameters.sampling_rate) - round(x_new(1) * EMGdata.parameters.sampling_rate));
EMGdata.trials.edited(a,1) = EMGdata.trials.edited(a,1)+1;
handles.EMGdata = EMGdata;
plot_figure(EMGdata,handles,a)
...
EMGdata.trials.ch4_EMG_RT(a,1) = EMGdata.trials.ch4_EMGburst_onset(a,1) - EMGdata.trials.go_stim_onset(a,1);
EMGdata.trials.ch2_EMGburst_area(a,1) = sum(abs(EMGrange)) / (round(x_new(2) * EMGdata.parameters.sampling_rate) - round(x_new(1) * EMGdata.parameters.sampling_rate));
EMGdata.trials.edited(a,1) = EMGdata.trials.edited(a,1)+1;
handles.EMGdata = EMGdata;
plot_figure(EMGdata,handles,a)
...
for n = 1:length(EMGdata.parameters.EMG_burst_channels)
    if EMGdata.trials.(['ch', num2str(EMGdata.parameters.EMG_burst_channels(n)),'_EMG_RT'])(a,1)
        EMGdata.trials.(['ch', num2str(EMGdata.parameters.EMG_burst_channels(n)),'_EMG_RT'])(a,1) = ...
            EMGdata.trials.(['ch', num2str(EMGdata.parameters.EMG_burst_channels(n)),'_EMGburst_onset'])(a,1) - EMGdata.trials.go_stim_onset(a,1); %%%%MF
    end
end
...
EMGdata.trials.go_stim_onset(a,1)=0; %%%%MF
EMGdata.trials.stop_stim_onset(a,1)=0; %%%%MF
EMGdata.trials.edited(a,1) = EMGdata.trials.edited(a,1)+1;
